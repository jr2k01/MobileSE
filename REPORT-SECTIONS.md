...
| 17.8 | 2026-08-28 | Push notifications work, and were shown to work on both devices | A workout logged on one device reaches the other in about twelve seconds, with the receiving app **not running** | The cause of the long silence was not code. `app/google-services.json` sat in `.gitignore` and everyone was told to create their own Firebase project — but the edge function holds the service-account key of exactly *one* project, so tokens from any other project cannot be addressed. Nothing arrives, and neither side logs anything unusual. The client file now lives in the repository, so the whole team uses `crewfit-6342e` |
...
| Push notifications | Nothing arrived, on any device but the one where the edge function had been set up — and without a single error message anywhere | A workout on one device notifies the crew on the other: *"Timo Kosowski trained Gym for 27 minutes"*, about twelve seconds later, with the receiving app killed beforehand. Verified in both directions | `31b56b8` |
