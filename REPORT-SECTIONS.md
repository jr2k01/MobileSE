...
| **Supabase Realtime** (removed 13 Aug) | Reload on `onResume` | It kept a WebSocket open and cost battery, while no screen listened for live updates. |
...
| **Runtime permissions** | `WorkoutTrackingActivity` (camera, microphone, location), `MainHubActivity` (Health Connect) | Android requires these to be requested at runtime, and every one of them can be refused. |
